...
int open_t(const char *pathname, int flags,mode_t mode){
    ...
    //printf("%s\n",keyword);
    while(getline(&line_open,&len_open,config_fp)!=-1) if(strstr(line_open,"BEGIN open-blacklist")!=NULL) break;
    while(getline(&line_open,&len_open,config_fp)!=-1){
        if(strstr(line_open,keyword)!=NULL){
            errno=EACCES;
            dprintf(log,"[logger] open (\"%s\",%d,%d) = -1 \n",pathname,flags,mode);
            return -1;
        }
        if(strstr(line_open,"END open-blacklist")!=NULL) break;
    }
    /*
    while(getline(&line_open,&len_open,config_fp)!=-1){
...
ssize_t read_t(int fd,void*buf,size_t count){
    ...
    while(getline(&line_read,&len_read,fp_read)!=-1) if(strstr(line_read,"BEGIN read-blacklist")!=NULL) break;
    while(getline(&line_read,&len_read,fp_read)!=-1){
        ...
        //if(strncmp(temp,"BEGIN read-blacklist",20)==0){
            ...
            //printf("match\n");
        //}
        if(strstr(line_read,"END read-blacklist")!=NULL) break;
        memset(block_arr[flag],0,strlen(line_read));
        memcpy(block_arr[flag],line_read,strlen(line_read));
        if(strstr(buf,line_read)!=NULL){
            errno=EIO;
            //printf("block\n");
...
int connect_t(int sockfd, const struct sockaddr *addr, socklen_t addrlen){
    ...
    strcpy(ip,ip_s);
    //printf("%s\n",ip);
    char*con_env=getenv("SANDBOX_CONFIG");
    FILE*fp_con=fopen(con_env,"r");
    char *keyword[1024];
    char*line_con=NULL;
    size_t len_con=0;
    int flag=0;
    ...
    char* LOGGER_FD=getenv("LOGGER_FD");
    long log=strtol(LOGGER_FD,NULL,10);
    while(getline(&line_con,&len_con,fp_con)!=-1) if(strstr(line_con,"BEGIN connect-blacklist")!=NULL) break;
    while(getline(&line_con,&len_con,fp_con)!=-1){
        ...
        //strncpy(temp,line_get,strlen(line_get)-2);
        //printf("strlen:%d (temp)\n",strlen(temp));
        if(strstr(line_con,"END connect-blacklist")!=NULL) break;
        
            //printf("ip:%s  temp:%s\n",ip,line_con);
                char *tt=strtok(line_con,":");
                //printf("tt:%s\n",tt);
                struct hostent *host;
                ...
                for (i = 0; addr_list[i] != NULL; i++) {
                    //fprintf(stderr,"ip:%s  addr_list:%s\n",ip,inet_ntoa(*addr_list[i]));
                    if(strstr(ip,inet_ntoa(*addr_list[i]))!=NULL){
                        //printf("fail connect\n");
...
int getaddrinfo_t(const char *node, const char *service, const struct addrinfo *hints, struct addrinfo **res){
    ...
    char* LOGGER_FD=getenv("LOGGER_FD");
    long log=strtol(LOGGER_FD,NULL,10);
    while(getline(&line_get,&len_get,fp_get)!=-1) if(strstr(line_get,"BEGIN getaddrinfo-blacklist")!=NULL) break;
    while(getline(&line_get,&len_get,fp_get)!=-1){
        //printf("strlen:%d line : %s\n",strlen(line_get),line_get);
        if(strstr(line_get,"END getaddrinfo-blacklist")!=NULL) break;
        
            if(strstr(line_get,node)!=NULL){
